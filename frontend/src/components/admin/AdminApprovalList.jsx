...
export const AdminApprovalList = ({ courses = [], onApprove, onReject, onView }) => {
  const getDeliveryBadge = (type) => {
    const deliveryMap = {
      online: { text: 'Online', class: 'bg-sky-500/10 text-sky-400 border-sky-500/20' },
      offline: { text: 'Offline', class: 'bg-amber-500/10 text-amber-400 border-amber-500/20' },
      blended: { text: 'Blended', class: 'bg-purple-500/10 text-purple-400 border-purple-500/20' },
    };
    const style = deliveryMap[type] || { text: type, class: 'bg-slate-500/10 text-slate-400 border-slate-500/20' };
    return (
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {courses.length === 0 ? (
        ...
        courses.map((course, index) => (
          ...
          >
            <BezelCard
              outerClassName="h-full hover:border-[#3B82F6]/30 hover:scale-[1.01] transition-all duration-300"
            ...
            >
              ...
              <div className="p-6 flex-1 flex flex-col justify-between">
                <div>
                  <h4 className="text-base font-bold text-white leading-snug tracking-tight mb-2 truncate-2-lines min-h-[44px]">
                    {course.title}
                  </h4>

                  <div className="space-y-2 mt-4 text-xs text-slate-400 font-mono">
                ...
                <div className="flex items-center gap-3 mt-6 pt-4 border-t border-slate-900">
                  <button
                    ...
                    className="flex-1 inline-flex items-center justify-center gap-1.5 px-3 py-2 rounded-full border border-slate-800 bg-slate-900/60 hover:bg-slate-800 text-xs font-semibold text-slate-300 transition-colors duration-200 active:scale-[0.98]"
                  >
                  ...
                  <button
                    ...
                    className="w-9 h-9 rounded-full bg-emerald-500/10 border border-emerald-500/30 flex items-center justify-center text-emerald-400 hover:bg-emerald-500 hover:text-white transition-all duration-300 active:scale-[0.9]"
                    title="Duyệt"
                  >
                  ...
                  <button
                    ...
                    className="w-9 h-9 rounded-full bg-rose-500/10 border border-rose-500/30 flex items-center justify-center text-rose-400 hover:bg-rose-500 hover:text-white transition-all duration-300 active:scale-[0.9]"
                    title="Từ chối"
                  >
                    <X className="w-4 h-4" />
                  </button>
