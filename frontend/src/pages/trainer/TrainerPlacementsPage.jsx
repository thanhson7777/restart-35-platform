...
const PlacementStatusModal = ({ isOpen, onClose, placement, onSave }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
      <div className="w-full max-w-md bg-[#111827] border border-slate-800 rounded-2xl shadow-2xl">
        <div className="flex items-center justify-between border-b border-slate-800 p-5">
          <div>
            <h3 className="text-base font-bold text-white">Cập nhật trạng thái</h3>
            <p className="text-xs text-slate-400 mt-0.5">{placement?.user?.displayName} - {placement?.company}</p>
          </div>
          ...
          {STATUS_OPTIONS.map((opt) => (
              ...
              className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl border transition-all duration-150 ${
                newStatus === opt.value
                  ? opt.color === 'amber' ? 'bg-amber-500/15 border-amber-500/40 text-amber-400' :
                    opt.color === 'blue' ? 'bg-blue-500/15 border-blue-500/40 text-blue-400' :
                    opt.color === 'purple' ? 'bg-purple-500/15 border-purple-500/40 text-purple-400' :
                    opt.color === 'green' ? 'bg-green-500/15 border-green-500/40 text-green-400' :
            ...
            >
              <span className={`w-3 h-3 rounded-full shrink-0 ${
                ...
                opt.color === 'purple' ? 'bg-purple-400' :
        ...
        <div className="flex items-center justify-end gap-3 border-t border-slate-800 p-5">
          ...
          <Button
            ...
            className="bg-blue-600 hover:bg-blue-700 text-white border-none text-sm py-2 px-5 font-semibold"
          >
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-2">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-extrabold tracking-tight text-white">Quản lý việc làm</h1>
          ...
          <Button
            ...
            className="bg-blue-600 hover:bg-blue-700 text-white border-none gap-2 text-sm py-2.5 px-4 font-semibold rounded-xl"
          >
            <Plus size={16} />
            Thêm Placement
          </Button>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {stats.map((stat) => {
          const colorMap = {
            ...
            gray: 'border-slate-500/20 bg-slate-500/5'
          };
          const iconMap = {
            blue: 'text-blue-400',
          ...
          return (
            <div
              key={stat.label}
              className={`rounded-xl border bg-[#111827] p-4 ${colorMap[stat.color]}`}
            >
              <div className="flex items-center justify-between">
                <div>
                ...
                <div className={`w-10 h-10 rounded-xl flex items-center justify-center bg-[#0c101d] ${iconMap[stat.color]}`}>
                  <stat.icon size={20} />
                </div>
              </div>
            </div>
      ...
      {detailPlacement && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
          <div className="w-full max-w-lg bg-[#111827] border border-slate-800 rounded-2xl shadow-2xl">
            <div className="flex items-center justify-between border-b border-slate-800 p-5">
              <div>
                <h3 className="text-base font-bold text-white">Chi tiết Placement</h3>
            ...
            <div className="p-5 space-y-4">
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  <p className="text-slate-500 text-xs">Học viên</p>
                  <p className="text-white font-semibold">{detailPlacement.user?.displayName || 'N/A'}</p>
                </div>
                <div>
                  <p className="text-slate-500 text-xs">Khóa học</p>
                  <p className="text-white font-semibold">{detailPlacement.course?.title || 'N/A'}</p>
                </div>
                <div>
                  <p className="text-slate-500 text-xs">Công ty</p>
                  <p className="text-white font-semibold">{detailPlacement.company || 'N/A'}</p>
                </div>
                <div>
                  <p className="text-slate-500 text-xs">Vị trí</p>
                  <p className="text-white font-semibold">{detailPlacement.position || 'N/A'}</p>
                </div>
                <div>
                  <p className="text-slate-500 text-xs">Lương</p>
                ...
                <div>
                  <p className="text-slate-500 text-xs">Loại công việc</p>
                  <p className="text-white font-semibold">{detailPlacement.employmentType || 'N/A'}</p>
                </div>
                {detailPlacement.startedDate && (
                  <div>
                    <p className="text-slate-500 text-xs">Ngày bắt đầu</p>
                ...
                {detailPlacement.notes && (
                  <div className="col-span-2">
                    <p className="text-slate-500 text-xs">Ghi chú</p>
                    <p className="text-slate-300 text-sm">{detailPlacement.notes}</p>
                  </div>
                )}
              </div>
            </div>
            <div className="flex items-center justify-end gap-3 border-t border-slate-800 p-5">
              ...
              <Button
                ...
                className="bg-blue-600 hover:bg-blue-700 text-white border-none text-sm font-semibold"
              >
                Cập nhật trạng thái
              </Button>
            </div>
