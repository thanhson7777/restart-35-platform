...
import {
  Camera, Save, Building2, Briefcase, Mail, Phone, MapPin, Hash, Users,
...
export default function TrainerProfilePage() {
  ...
  useEffect(() => {
    const fetchOrgData = async () => {
      ...
      try {
        ...
        toast.error(error?.response?.data?.message || 'Không thể tải thông tin trung tâm đào tạo');
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const handleSaveOrg = async (e) => {
    ...
    if (!orgData.name.trim()) {
      errors.name = orgData.trainerType === 'individual'
        ? 'Tên chuyên gia hiển thị là bắt buộc'
        : 'Tên trung tâm đào tạo là bắt buộc';
    }
    setOrgErrors(errors);
    ...
    try {
      const payload = {
        ...
        identityNumber: orgData.identityNumber.trim(),
        trainerType: orgData.trainerType,
      };
      ...
      toast.success(orgData.trainerType === 'individual'
        ? 'Cập nhật hồ sơ chuyên gia thành công!'
        : 'Cập nhật hồ sơ trung tâm đào tạo thành công!');
    } catch (err) {
  ...
  const handleSaveUser = async (e) => {
    e.preventDefault();
    const errors = {};
    if (!userData.displayName.trim()) errors.displayName = 'Tên giảng viên là bắt buộc';
    ...
    try {
      ...
      toast.error(err?.data?.message || 'Lỗi cập nhật thông tin giảng viên');
    } finally {
  ...
  const isIndividual = currentUser?.organization?.trainerType === 'individual' || orgData.trainerType === 'individual';
    ...
    <div className="w-full space-y-6 pb-12">
      <div>
        ...
        <p className="text-[hsl(var(--admin-text-muted))] mt-1">
          {hasOrg
            ? (isIndividual ? 'Quản lý thông tin hiển thị chuyên môn và bảo mật tài khoản giảng viên.' : 'Quản lý thông tin hiển thị của trung tâm đào tạo và bảo mật tài khoản giảng viên.')
            : 'Quản lý thông tin hiển thị và bảo mật tài khoản giảng viên.'}
        </p>
      </div>
        ...
        <TabsList className={`grid w-full ${hasOrg ? 'grid-cols-3' : 'grid-cols-2'} mb-6 bg-[hsl(var(--admin-surface))] border border-[hsl(var(--admin-border))] p-1 rounded-xl`}>
          {hasOrg && (
            <TabsTrigger value="organization" className="gap-2 rounded-lg data-[state=active]:bg-[hsl(var(--admin-accent))] data-[state=active]:text-white">
              <Building2 size={16} />
              {isIndividual ? 'Hồ sơ Chuyên gia' : 'Trung tâm Đào tạo'}
            </TabsTrigger>
          )}
                  ...
                  <Card className="bg-[hsl(var(--admin-surface))] border border-[hsl(var(--admin-border))] rounded-2xl overflow-hidden">
                    <CardHeader className="bg-[hsl(var(--admin-surface-elevated))] border-b border-[hsl(var(--admin-border))]">
                      <CardTitle className="flex items-center gap-2 text-lg text-[hsl(var(--admin-text-primary))]">
                        ...
                        {isIndividual ? 'Thông tin cơ bản Chuyên gia' : 'Thông tin cơ bản Trung tâm'}
                      </CardTitle>
                    </CardHeader>
                    <CardContent className="p-6 space-y-6">
                      ...
                      <div className="flex flex-col sm:flex-row items-center gap-6 pb-6 border-b border-[hsl(var(--admin-border))]">
                        ...
                        <div>
                          <p className="text-sm font-medium text-[hsl(var(--admin-text-primary))]">
                            {isIndividual ? 'Ảnh chân dung chuyên gia' : 'Logo Trung tâm'}
                          </p>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mt-6">
                        <div className={isIndividual ? "col-span-1" : "md:col-span-2"}>
                          <IconInput
                            label={isIndividual ? "Tên chuyên môn hiển thị" : "Tên trung tâm đào tạo"}
                            id="orgName"
                            icon={Building2}
                            value={orgData.name}
                            onChange={(e) => handleOrgChange('name', e.target.value)}
                            error={orgErrors.name}
                            placeholder={isIndividual ? "VD: Thầy Nguyễn Văn A..." : "Trung tâm Đào tạo nghề..."}
                            required
                          />
                        </div>
                        ...
                        {isIndividual ? (
                          <IconInput
                            ...
                            id="identityNumber"
                            icon={Hash}
                            value={orgData.identityNumber}
                            onChange={(e) => handleOrgChange('identityNumber', e.target.value)}
                            placeholder="Nhập số giấy tờ cá nhân"
                          />
                        ) : (
                          <IconInput
                            label="Mã số thuế / Giấy chứng nhận"
                            id="taxCode"
                            icon={Hash}
                            value={orgData.taxCode}
                            onChange={(e) => handleOrgChange('taxCode', e.target.value)}
                            placeholder="Mã số thuế hoặc số quyết định thành lập"
                          />
                        )}

                        <IconInput
                          label={isIndividual ? "Lĩnh vực giảng dạy chính" : "Lĩnh vực đào tạo chính"}
                          id="industry"
                          icon={Briefcase}
                          value={orgData.industry}
                          onChange={(e) => handleOrgChange('industry', e.target.value)}
                          placeholder="VD: May mặc, Cơ khí, CNTT..."
                        />
                        ...
                        <div className="md:col-span-2">
                          <IconInput
                            label={isIndividual ? "Địa chỉ thường trú / hoạt động" : "Địa chỉ trụ sở"}
                            id="address"
                            icon={MapPin}
          ...
          <Card className="bg-[hsl(var(--admin-surface))] border border-[hsl(var(--admin-border))] rounded-2xl overflow-hidden">
            <CardHeader className="bg-[hsl(var(--admin-surface-elevated))] border-b border-[hsl(var(--admin-border))]">
              <CardTitle className="flex items-center gap-2 text-lg text-[hsl(var(--admin-text-primary))]">
                ...
                Tài khoản Giảng viên / Người đại diện
              </CardTitle>
            </CardHeader>
              ...
              <motion.div variants={containerVariants} initial="hidden" animate="visible" className="space-y-6">
                ...
                <motion.div variants={itemVariants}>
                  <div className="flex items-start gap-3 p-4 bg-blue-50/50 dark:bg-blue-900/10 rounded-xl border border-blue-200 dark:border-blue-900/50">
                    ...
                    <div>
                      <p className="text-sm font-medium text-blue-700 dark:text-blue-400">Bảo mật tài khoản giảng viên</p>
                      <p className="text-xs text-blue-600/80 dark:text-blue-400/80 mt-1">
                        Sử dụng mật khẩu mạnh gồm chữ hoa, chữ thường, số và ký tự đặc biệt để đảm bảo an toàn cho tài khoản giảng viên của bạn.
                      </p>
                    </div>
                  </div>
                </motion.div>
