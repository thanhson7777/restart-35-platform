...
import { 
  Camera, Save, Building2, Briefcase, Mail, Phone, MapPin, Hash, Target, Map,
  User, Lock, AlertCircle, PhoneIcon, UserIcon, MailIcon
} from 'lucide-react';
...
import { NGO_FOCUS_AREAS_OPTIONS, VIETNAM_PROVINCES } from '@/data/profileData';

// Map focus area codes to Vietnamese display labels
const getFocusAreasDisplay = (focusAreasArray) => {
  if (!Array.isArray(focusAreasArray)) return '';
  return focusAreasArray.map(val => {
    const found = NGO_FOCUS_AREAS_OPTIONS.find(opt => opt.value === val || opt.label === val);
    return found ? found.label : val;
  }).join(', ');
};

// Map display labels back to codes for API payload
const parseFocusAreasPayload = (focusAreasString) => {
  return focusAreasString.split(',').map(s => {
    const trimmed = s.trim();
    const found = NGO_FOCUS_AREAS_OPTIONS.find(opt => opt.label.toLowerCase() === trimmed.toLowerCase() || opt.value === trimmed);
    return found ? found.value : trimmed;
  }).filter(Boolean);
};

// Map operating region codes to Vietnamese province labels
const getOperatingRegionsDisplay = (regionsArray) => {
  if (!Array.isArray(regionsArray)) return '';
  return regionsArray.map(val => {
    const found = VIETNAM_PROVINCES.find(opt => opt.value === val || opt.label === val);
    return found ? found.label : val;
  }).join(', ');
};

// Map display labels back to codes for API payload
const parseOperatingRegionsPayload = (regionsString) => {
  return regionsString.split(',').map(s => {
    const trimmed = s.trim();
    const found = VIETNAM_PROVINCES.find(opt => opt.label.toLowerCase() === trimmed.toLowerCase() || opt.value === trimmed);
    return found ? found.value : trimmed;
  }).filter(Boolean);
};
...
export default function NgoProfilePage() {
    ...
    const fetchOrgData = async () => {
      ...
      try {
        ...
        setOrgData({
          ...
          focusAreas: getFocusAreasDisplay(data.focusAreas),
          operatingRegions: getOperatingRegionsDisplay(data.operatingRegions),
        });
      } catch (error) {
        toast.error(error?.response?.data?.message || 'Không thể tải thông tin tổ chức NGO');
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const handleSaveOrg = async (e) => {
    e.preventDefault();
    const errors = {};
    if (!orgData.name.trim()) errors.name = 'Tên tổ chức NGO là bắt buộc';
    setOrgErrors(errors);
    ...
    try {
      const payload = {
        ...
        focusAreas: parseFocusAreasPayload(orgData.focusAreas),
        operatingRegions: parseOperatingRegionsPayload(orgData.operatingRegions),
      };
      ...
      toast.success('Cập nhật hồ sơ tổ chức NGO thành công!');
    } catch (err) {
      toast.error(err?.response?.data?.message || 'Lỗi cập nhật tổ chức NGO');
    } finally {
  ...
  if (isLoading && currentUser?.organizationId) {
    return (
  ...
  const hasOrg = !!currentUser?.organizationId;
  const defaultTab = hasOrg ? 'organization' : 'user';
    ...
    <div className="w-full space-y-6 pb-12">
      <div>
        ...
        <p className="text-[hsl(var(--admin-text-muted))] mt-1">
          {hasOrg 
            ? 'Quản lý thông tin hiển thị của tổ chức NGO và bảo mật tài khoản người đại diện.'
            : 'Quản lý thông tin hiển thị và bảo mật tài khoản.'}
        </p>
      </div>

      <Tabs key={defaultTab} defaultValue={defaultTab} className="w-full">
        <TabsList className={`grid w-full ${hasOrg ? 'grid-cols-3' : 'grid-cols-2'} mb-6 bg-[hsl(var(--admin-surface))] border border-[hsl(var(--admin-border))] p-1 rounded-xl`}>
          {hasOrg && (
          ...
          <TabsTrigger value="user" className="gap-2 rounded-lg data-[state=active]:bg-[hsl(var(--admin-accent))] data-[state=active]:text-white">
            <User size={16} />
            {hasOrg ? 'Người đại diện' : 'Thông tin cá nhân'}
          </TabsTrigger>
              ...
              <motion.div variants={containerVariants} initial="hidden" animate="visible" className="space-y-6">
                  ...
                  <Card className="bg-[hsl(var(--admin-surface))] border border-[hsl(var(--admin-border))] rounded-2xl overflow-hidden">
                    ...
                    <CardContent className="p-6 space-y-6">
                      ...
                      <div className="flex flex-col sm:flex-row items-center gap-6 pb-6 border-b border-[hsl(var(--admin-border))]">
                        ...
                        <div>
                          <p className="text-sm font-medium text-[hsl(var(--admin-text-primary))]">Logo Tổ chức NGO</p>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mt-6">
                        ...
                        <IconInput
                          label="Giấy phép hoạt động / Quyết định thành lập / Mã số thuế"
                          id="taxCode"
                          icon={Hash}
                          value={orgData.taxCode}
                          onChange={(e) => handleOrgChange('taxCode', e.target.value)}
                          placeholder="Nhập mã số giấy phép hoặc mã số thuế"
                        />

                        <IconInput
                          label="Sứ mệnh / Lĩnh vực hỗ trợ trọng tâm"
                          id="focusAreas"
                          icon={Target}
                          value={orgData.focusAreas}
                          onChange={(e) => handleOrgChange('focusAreas', e.target.value)}
                          placeholder="VD: Hỗ trợ sinh kế, Người khuyết tật, Giáo dục (Phân cách bằng dấu phẩy)"
                        />

                        <div className="md:col-span-2">
                          <IconInput
                            label="Địa bàn hoạt động chính"
                            id="operatingRegions"
                            icon={Map}
                            value={orgData.operatingRegions}
                            onChange={(e) => handleOrgChange('operatingRegions', e.target.value)}
                            placeholder="VD: Tây Nguyên, Miền Tây, Hà Nội (Phân cách bằng dấu phẩy)"
                          />
                        </div>
                ...
                <motion.div variants={itemVariants} className="flex justify-end pt-2">
                  <Button type="submit" isLoading={isSavingOrg} size="lg" className="gap-2 bg-[hsl(var(--admin-accent))] hover:bg-[hsl(var(--admin-accent-hover))] text-white px-8">
                    <Save size={16} /> Lưu thay đổi Tổ chức
                  </Button>
                </motion.div>
              </motion.div>
            </form>
          ...
          <Card className="bg-[hsl(var(--admin-surface))] border border-[hsl(var(--admin-border))] rounded-2xl overflow-hidden">
            <CardHeader className="bg-[hsl(var(--admin-surface-elevated))] border-b border-[hsl(var(--admin-border))]">
              <CardTitle className="flex items-center gap-2 text-lg text-[hsl(var(--admin-text-primary))]">
                ...
                Tài khoản Người đại diện tổ chức
              </CardTitle>
            </CardHeader>
              ...
              <motion.div variants={containerVariants} initial="hidden" animate="visible" className="space-y-6">
                ...
                <motion.div variants={itemVariants}>
                  <div className="flex items-start gap-3 p-4 bg-blue-50/50 dark:bg-blue-900/10 rounded-xl border border-blue-200 dark:border-blue-900/50">
                    ...
                    <div>
                      <p className="text-sm font-medium text-blue-700 dark:text-blue-400">Bảo mật tài khoản NGO</p>
                      <p className="text-xs text-blue-600/80 dark:text-blue-400/80 mt-1">
                        Sử dụng mật khẩu mạnh gồm chữ hoa, chữ thường, số và ký tự đặc biệt để đảm bảo an toàn cho tài khoản NGO của bạn.
                      </p>
                    </div>
                  </div>
                </motion.div>
