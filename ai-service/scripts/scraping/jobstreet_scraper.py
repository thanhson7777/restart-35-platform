# -*- coding: utf-8 -*-
"""
JobStreet.vn Scraper - Trang tuyển dụng hàng đầu Đông Nam Á

JobStreet sử dụng JavaScript để render job listings,
...
class JobStreetScraper:
    """
    JobStreet.vn scraper sử dụng Playwright
    ...
    SEARCH_URL = 'https://www.jobstreet.vn/jobs'

    # Job categories on JobStreet
    CATEGORIES = {
        'accounting': 'Kế toán',
        ...
        'marketing-communications': 'Marketing - Truyền thông',
        'media-creative-design': 'Media - Sáng tạo - Thiết kế',
        'medical-practitioners': 'Y tế',
        'professional-services': 'Dịch vụ chuyên nghiệp',
        'property-real-estate': 'Bất động sản',
        'retail-consumer-products': 'Bán lẻ - Hàng tiêu dùng',
        'sales-bd': 'Kinh doanh - Phát triển kinh doanh',
        'sciences': 'Khoa học',
        'secretarial-executive': 'Thư ký - Hành chính',
        'services': 'Dịch vụ',
        'supply-chain-procurement': 'Chuỗi cung ứng - Mua hàng',
        'transport-warehousing': 'Vận tải - Kho vận',
    }
    ...
    def _wait_for_jobs(self, timeout: int = 10000):
        ...
        try:
            self.page.wait_for_selector('.job, [data-job-id], .job-card, article', timeout=timeout)
            return True
        except:
            ...
            if matches:
                try:
                    if pattern_type == 'usd_range':
                        min_v = float(matches[0][0].replace(',', '')) * 25_000_000
                        max_v = float(matches[0][1].replace(',', '')) * 25_000_000
    ...
    def _parse_job_card(self, card) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # Title
            title_elem = (
                card.query_selector('h3 a, h2 a, .title a, a[class*="title"], a[class*="job"]') or
            ...
            # Company
            company_elem = card.query_selector('[class*="company"], [class*="employer"], [class*="brand"]')
            job['company'] = company_elem.text_content().strip() if company_elem else ''

            # Salary
            salary_elem = card.query_selector('[class*="salary"], [class*="price"], [class*="compensation"]')
            ...
            # Location
            location_elem = card.query_selector('[class*="location"], [class*="address"], [class*="city"]')
            job['location'] = self._parse_location(
                location_elem.text_content().strip() if location_elem else ''
            )

            # Experience
            exp_elem = card.query_selector('[class*="experience"], [class*="level"], [class*="seniority"]')
            job['experience_required'] = self._parse_experience(
                exp_elem.text_content().strip() if exp_elem else ''
            )

            # Skills
            skill_elems = card.query_selector_all('[class*="skill"], [class*="tag"], [class*="keyword"]')
            job['skills'] = '|'.join(self._extract_skills(skill_elems))

            # Job type
            type_elem = card.query_selector('[class*="type"], [class*="employment"]')
            job['type'] = self._parse_job_type(
                type_elem.text_content().strip() if type_elem else ''
            )

            # Description
            desc_elem = card.query_selector('p, [class*="desc"], [class*="summary"]')
    ...
    def scrape_page(self, url: str, retries: int = 3) -> List[Dict[str, Any]]:
        ...
        debug_file = Path('data/debug_jobstreet.html')
    ...
    def scrape_by_keyword(self, keyword: str, pages: int = 5) -> List[Dict[str, Any]]:
        ...
        base_url = f"{self.SEARCH_URL}?keywords={encoded_keyword}"
    ...
    def scrape_by_location(self, location: str, pages: int = 5) -> List[Dict[str, Any]]:
        ...
        base_url = f"{self.SEARCH_URL}?location={loc_slug}"
    ...
    def scrape(self, pages: int = 5, **kwargs) -> List[Dict[str, Any]]:
        ...
        self.logger.info("Scraping JobStreet.vn main listings...")
        jobs = self.scrape_all(pages=min(pages, 3))  # Limit main scrape
        all_jobs.extend(jobs)

        # Also search for 35+ suitable keywords
        keywords = ['accounting', 'admin', 'hr', 'sales', 'customer service']
        for keyword in keywords[:3]:
            self.logger.info(f"\nSearching for: {keyword}")
            try:
                jobs = self.scrape_by_keyword(keyword, pages=min(pages, 2))
                all_jobs.extend(jobs)
            except Exception as e:
    ...
    def log_stats(self):
        """Log statistics"""
        self.logger.info("=" * 50)
        self.logger.info(f"JobStreet Scraper Statistics:")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='JobStreet.vn Playwright Scraper')
    ...
    parser.add_argument('--output', type=str, default='jobstreet_jobs.json', help='Output file')
    ...
    try:
        print("\n[START] JobStreet.vn Playwright Scraper")
        print("=" * 50)
        ...
        # Scrape
        if args.keyword:
            jobs = scraper.scrape_by_keyword(args.keyword, pages=args.pages)
        elif args.location:
